...
<head>
	...
	window.onload = function() {
		console.log( "customCompGradeUpdateList.onload()" );
  	...
  	//조건조회
	let searchCustomCompGradeUpdateList = function() {
  		
		updateGradeForm.action = '${ctxPath}/admin/customCompGradeUpdateList';
		updateGradeForm.submit();

		console.log( "customCompGradeUpdateList.searchCustomCompGradeUpdateList()" );
	}
	let $btnFSearch = $( "#fSearch" ); //jquery 이벤트 핸들러를 위한 변수
	$btnFSearch.on( "click", searchCustomCompGradeUpdateList ); //jquery 이벤트 핸들러 on 적용, 처리핸들링함수 추가
  	
	//수정버튼 - jquery on이벤트 함수 적용
	let updateManyCustomCompLevel = function() {
		console.log( "customCompGradeUpdateList.updateManyCustomCompLevel()" );
		...
		if (confirm('선택한 회원들의 고객등급을 수정합니까?')) {
			updateGradeForm.action = '${ctxPath}/admin/customCompGradeUpdate';
			updateGradeForm.submit();
		}
	} 
	let $btnFUpdate = $( "#fUpdate" );//jquery 이벤트 핸들러를 위한 변수
	$btnFUpdate.on( "click", updateManyCustomCompLevel ); //jquery 이벤트 핸들러 on 적용, 처리핸들링함수 추가
	
	//전체체크박스버튼 - jquery on이벤트 함수
	let addAllUpdateHtml = function() {
		console.log( "customCompGradeUpdateList.addAllUpdateHtml()" );
	...
	let addUpdateHtml = function( customId, customGradeCd, idx, isChecked, isDistinct ) {
		console.log( "customCompGradeUpdateList.addUpdateHtml()" );
...
//alert(updCnt);	
	...
	//개별수정
	let updateOnceCustomCompLevel = function(customId, customGradeCd) {
 		console.log( "customCompGradeUpdateList.updateOnceCustomCompLevel()" );
	...
	let replaceAllCustomGrade = function(orgAllCustomGrade) {
		console.log( "customCompGradeUpdateList.replaceAllCustomGrade()" );
...
<body>
  ...
  <p><br></p>
  <div class="container">
  	<h3 class="text-center">기업고객 고객등급수정목록</h3>
...
--%>
		  		&nbsp;<input type="button" id="fSearch" value="조회" class="btn btn-primary" onclick="searchCustomCompGradeUpdateList()" /> 
		  		&nbsp;<input type="button" id="fChange" value="적용" class="btn btn-primary" onclick="replaceAllCustomGrade('${customGrade}')" />
		  		&nbsp;<input type="button" id="fUpdate" value="수정" class="btn btn-primary" onclick="updateManyCustomCompLevel()" />
	  	</div></div>
	  		...
	  		<c:if test="${! empty vos}">
 				<c:forEach var="vo" items="${vos}" begin="0" step="1" varStatus="st">
					...
					<tr>
						...
						<td><input type="button" value="수정" class="btn btn-primary btn-sm" onclick="javascript:updateOnceCustomCompLevel('${vo.customId}', $('#customGrade${st.count}').val())" /></td>
					</tr>
				</c:forEach>
	  		</c:if>
